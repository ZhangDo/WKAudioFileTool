...
@implementation WKAudioParsedTool
...
@interface WKAudioFileStreamManager ()
{
@private
    BOOL _discontinuous;
...
- (void)wk_handleAudioFileStreamProperty:(AudioFileStreamPropertyID)propertyID;
- (void)wk_handleAudioFileStreamPackets:(const void *)packets
...
static void WKSAudioFileStreamPropertyListener(void *inClientData,
                                               AudioFileStreamID inAudioFileStream,
                                               AudioFileStreamPropertyID inPropertyID,
                                               UInt32 *ioFlags) {
    WKAudioFileStreamManager *audioFileStream = (__bridge WKAudioFileStreamManager *)inClientData;
    [audioFileStream wk_handleAudioFileStreamProperty:inPropertyID];
}

static void WKAudioFileStreamPacketsCallBack(void *inClientData,
                                             UInt32 inNumberBytes,
                                             UInt32 inNumberPackets,
                                             const void *inInputData,
                                             AudioStreamPacketDescription *inPacketDescriptions) {
    WKAudioFileStreamManager *audioFileStream = (__bridge WKAudioFileStreamManager *)inClientData;
    [audioFileStream wk_handleAudioFileStreamPackets:inInputData
...
@implementation WKAudioFileStreamManager
...
- (instancetype)initWithFileType:(AudioFileTypeID)fileType fileSize:(unsigned long long)fileSize error:(NSError **)error {
    ...
    if (self) {
        ...
        _fileType = fileType;
        _fileSize = fileSize;
        [self wk_openAudioFileStreamWithFileTypeHint:_fileType error:error];
    }
    return self;
}

- (void)dealloc {
    [self wk_closeAudioFileStream];
}
...
- (BOOL)wk_openAudioFileStreamWithFileTypeHint:(AudioFileTypeID)fileTypeHint error:(NSError *__autoreleasing *)error {
    OSStatus status = AudioFileStreamOpen((__bridge void *)self,
                                          WKSAudioFileStreamPropertyListener,
                                          WKAudioFileStreamPacketsCallBack,
                                          fileTypeHint,
                                          &_audioFileStreamID);
...
- (void)calculateAudioDataLength {
    _audioDataLength = _audioDataByteCount / (self.format.mBitsPerChannel / 8.0) / self.format.mChannelsPerFrame;
}
...
- (void)wk_handleAudioFileStreamPackets:(const void *)packets
    ...
    for (int i = 0; i < numberOfPackets; ++i) {
        ...
        WKAudioParsedTool *parsedData = [WKAudioParsedTool wk_parsedAudioDataWithBytes:packets + packetOffset
